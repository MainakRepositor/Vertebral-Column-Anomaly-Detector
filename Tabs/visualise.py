...
def app(df, X, y):
    ...
    # Set the page title
    st.title("Visualise the Vertebral Column Anomaly Detection Parameters")
    ...
    if st.checkbox("Pelvic Incidence Level vs Class of Vertebrae"):
        sns.color_palette("rocket", as_cmap=True)
        ax=sns.scatterplot(x="pelvic_incidence",y="class",data=df)
        st.pyplot()

    if st.checkbox("Lumbar Lordosis Angle vs Class of Vertebrae"):
        sns.color_palette("rocket", as_cmap=True)
        ax=sns.scatterplot(x="lumbar_lordosis_angle",y="class",data=df)
        st.pyplot()

    if st.checkbox("Sacral Slope vs Class of Vertebrae"):
        sns.color_palette("rocket", as_cmap=True)
        ax=sns.scatterplot(x="sacral_slope",y="class",data=df)
        st.pyplot()

    if st.checkbox("Show Histogram"):
        sns.color_palette("rocket", as_cmap=True)
        ax=sns.histplot(data=df,x="degree",y="class")
        st.pyplot()
